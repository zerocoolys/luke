...
public class XMLExporter extends Observable {
	 ...
	 * @throws Exception
	 */
	public boolean export(OutputStream output, boolean decode, boolean preamble, boolean info, String rootElementName,
			Ranges ranges) throws Exception {
		running = true;
	...
	private void writeDoc(BufferedWriter bw, int docNum, Document doc, boolean decode, Bits liveDocs) throws Exception {
		...
		for (String fieldName : fieldNames) {
			...
			FieldInfo fieldInfo = fieldInfos.fieldInfo(fields[0].name());
			
//			DocValues dv = atomicReader.getNormValues(fields[0].name());
//			if (dv != null) {
//				// export raw value - we don't know what similarity was used
//				String type = fieldInfo.;
...
//			}
	...
	private void writeTermVector(BufferedWriter bw, Terms tfv, Bits liveDocs) throws Exception {
		...
		while (te.next() != null) {
			...
			offsets.setLength(0);
			DocsAndPositionsEnum newDpe = te.docsAndPositions(liveDocs, dpe, DocsAndPositionsEnum.FLAG_OFFSETS);
			if (newDpe == null) {
				continue;
			}
			dpe = newDpe;
	...
	private void writeIndexInfo(BufferedWriter bw) throws Exception {
		...
		if (dir != null) {
			...
			for (String file : files) {
				bw.write("  <file name='" + file + "' size='" + dir.fileLength(file) + "' func='"
						+ IndexGate.getFileFunction(file) + "'/>\n");
			}
			bw.write(" </files>\n");
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 2) {
			...
			System.err
					.println("\trange\tspecify ranges of documents to export. Expressions cannot contain whitespace!");
